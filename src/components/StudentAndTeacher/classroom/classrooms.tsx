...
const Classrooms = () => {
  ...
  useEffect(() => {
    const fetchClassroomsOfTeacher = async () => {
      try {
        if (teacher) {
    ...
    fetchClassroomsOfTeacher();
  }, [teacher, isCreatedClassroom]);

  const handleModalSubmit = async (data: ClassroomData) => {
    try {
      if (teacher) {
        data.teacherid = teacher._id;
      }
      const createClassroom = await ApiController.CreateClassroom(data);
      
      if (createClassroom.status === 200) {
        setIsCreatedClassroom(true);
        toast.success(createClassroom.data.message);
      } else {
        toast.error(createClassroom.response.data.error);
      }
    } catch (error) {
  ...
  const joinClassroom = async (classroomId: string, teacherId: string) => {
    try {
      const response = await ApiController.JoinClassroom( classroomId, teacherId );
      if (response.status === 200) {
          ...
          <div className="bg-white px-24 py-28 rounded-lg shadow-lg max-w-6xl w-full">
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-2xl font-bold text-gray-800">
                Welcome, {role === "Teacher" && teacher?.name ? teacher.name : "Teacher"}!
              </h1>
              {role === "Teacher" ? (
                <button onClick={() => setModalOpen(true)} className="border-2 border-[#2E236C] text-black font-semibold px-4 py-2 rounded-md hover:bg-[#544a8f] hover:text-white">
                  Create Classroom
                </button>
              ) : (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {classrooms.map((classroom) => (
                  ...
                  >
                    ...
                    <button onClick={()=>{joinClassroom(classroom._id,teacher._id)}}  className="absolute bottom-4 right-4 bg-[#2E236C] px-6 py-2 rounded-md text-white hover:bg-[#544a8f] transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-900">
                      {"Join"}
                    </button>
